...
class LayeredSequenceUnparser(
  ...
  override def unparse(state: UState): Unit = {
    // Layers are write-only: build only needs to navigate the layer body
    // to build the tree, so this skips withLayerTransform entirely, since
    // build's no-op-sink DOS can't survive the addBuffered() split it does.

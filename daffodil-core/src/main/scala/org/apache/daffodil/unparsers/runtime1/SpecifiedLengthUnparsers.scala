...
class SpecifiedLengthPrefixedUnparser(
  ...
  override def unparse(state: UState): Unit = {
    if (state.isBuildOnly) {
      // Build only needs to recurse into eUnparser to build the tree;
      // without this early return it would also create its own detached
      // prefix-length suspension, one that can never resolve.
      eUnparser.unparse1(state)
      return
    }
